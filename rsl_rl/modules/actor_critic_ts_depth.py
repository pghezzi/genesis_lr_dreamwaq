...
class ActorCriticTSDepth(nn.Module):
    ...
    def __init__(self,  
        ...
        self.depth_history_encoder = DepthHistoryEncoder(
            depth_image_resolution,
            num_actor_obs,
            num_latent_dims,
            cnn_input_channel,
        ...
        for l in range(len(actor_hidden_dims)):
            if l == len(actor_hidden_dims) - 1:
                ...
                actor_layers.append(activation_fn)
        # add final hardTanh to bound actions between -clip_actions and clip_actions
    ...
    def update_distribution(self, observations, depth_image_features, privilege_observations,
        ...
        if act_type == 'teacher':
            ...
            if masks is not None: # batch mode
    ...
    def act_student(self, observations, depth_image_features, **kwargs):
        ...
        actions_mean = self.actor(torch.cat(
            (observations, latent.squeeze(0)), dim=-1))
        return actions_mean
