...
class ActorCritic_DWAQ(nn.Module):
    ...
    def cenet_forward(self,obs_history):
        ...
        code = torch.nan_to_num(torch.cat((code_vel,code_latent),dim=-1), nan=0.001)
        decode = self.decoder(code)
        #for i, x in enumerate([
        #    distribution, mean_latent, logvar_latent, mean_vel, logvar_vel, code_latent, code_vel, code, decode 
        #]):
        #    print(f"{i+1}) {x}")
        print(f"distribution: {torch.any(torch.isnan(distribution))}")
        print(f"Latent v Vel: {torch.any(torch.isnan(code_latent))} {torch.any(torch.isnan(code_vel))} ")
        print(f"Decode: {torch.any(torch.isnan(code))}")
    ...
    def act(self, observations, obs_history, **kwargs):
        print(f"Observation(A): {observations}")
        print(f"Observation Hist: {torch.any(torch.isnan(obs_history))}")
        code,_,decode,_,_,_,_ = self.cenet_forward(obs_history)
        observations = torch.cat((code,observations),dim=-1)
        print("New Obs", observations)
