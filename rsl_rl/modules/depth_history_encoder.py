...
class DepthHistoryEncoder(nn.Module):
    """encoder that encodes depth image and observation history into a latent vector"""
    ...
    def __init__(self,
        ...
        # CNN for depth image encoding
        in_channels = cnn_input_channel
        in_height = depth_image_resolution[0]
        in_width = depth_image_resolution[1]
        cnn_layers = []
        # The resulted CNN architecture is like follows:
        # Conv2d -> MaxPool2d -> activation -> Conv2d -> activation -> ... -> Flatten -> FC -> activation -> ... -> FC -> activation
        for i, out_channels in enumerate(cnn_channel_dims):
            ...
            in_height = (in_height - cnn_kernel_sizes[i]) // cnn_strides[i] + 1 # after conv layer
            in_width = (in_width - cnn_kernel_sizes[i]) // cnn_strides[i] + 1
            if i == 0:
                cnn_layers.append(nn.MaxPool2d(kernel_size=2, stride=2))
                cnn_layers.append(activation_fn)
                in_height = (in_height - 2) // 2 + 1 # after pooling
        ...
        # Combination MLP, combine cnn encoding and observation
        ...
        if rnn_type.lower() == 'lstm':
            self.rnn = nn.LSTM(
                input_size=combination_mlp_dims[-1],
        ...
        # Output MLP
        self.latent_output_mlp = nn.Sequential(
            nn.Linear(rnn_hidden_size, num_latent_dims),
        )
    ...
    def forward(self, observation, depth_image, hidden_states=None, masks=None):
        ...
        if batch_mode:
            # record first(traj_len) and second(num_envs) dim of depth_image
            shape0 = depth_image.shape[0]
            shape1 = depth_image.shape[1]
            depth_image = depth_image.flatten(0, 1)  # merge first two dims for CNN processing
        # Encode depth image
        ...
        if not batch_mode:
            ...
            # view depth_encoding back to (traj_len, num_envs, feature_dim)
            depth_encoding = depth_encoding.view(shape0, shape1, -1)  # restore first two dims
        # Combine depth features and observation
    ...
    def reset_hidden_states(self, dones=None):
        # if hidden states have not been initialized yet, nothing to reset
