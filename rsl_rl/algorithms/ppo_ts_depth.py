...
class PPO_TSDepth(PPO):
    ...
    def act(self, obs, privileged_obs, depth_image_features, critic_obs):
        ...
        if not self.distillation:
            ...
            self.transition.hidden_states = self.actor_critic.get_hidden_states()
            student_actions = self.actor_critic.act(obs, depth_image_features, None,
                                                    "student", None, None).detach()
            # student_actions_deterministic = self.actor_critic.act_student(obs, depth_image_features)
            with torch.no_grad():
            ...
            # use deterministic student action as actually applied action
    ...
    def update(self):
        ...
        if not self.distillation: # phase 1 training, RL for teacher_ac, SL for student
            ...
            for obs_batch, privileged_obs_batch, depth_image_features_batch, critic_obs_batch, \
                ...
                advantages_batch, old_mu_batch, old_sigma_batch, hid_states_batch, masks_batch, \
                ...
                # add entropy loss to encourage convergence to teacher policy
