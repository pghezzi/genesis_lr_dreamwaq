...
def main():
    ...
    assert args.folder, "No folder provided"
    ...
    files = {
        ...
        if (path := folder / f"{name}.pt").is_file()
    }

    assert len(list(files.keys())) == 4, "Missing file"
    ...
    model_path = os.path.join(out_dir, f"selector_model_{folder.name}_acc_{int(temporal_accuracy)}_intacc_{int(instantaneous_accuracy)}_{timestamp}.pt")
