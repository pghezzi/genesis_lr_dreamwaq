...
def update_cfg_from_args(env_cfg, cfg_train, args):
    ...
    if env_cfg is not None:
        ...
        if args.num_envs is not None:
            ...
            # Viewer indices in several configs are generated for the default
            # environment count. Rebuild them after a CLI override so Genesis
            # never receives out-of-range environment IDs.
            if (
                hasattr(env_cfg, "viewer")
                and hasattr(env_cfg.viewer, "rendered_envs_idx")
            ):
                configured_render_count = len(
                    env_cfg.viewer.rendered_envs_idx
                )
                env_cfg.viewer.rendered_envs_idx = list(
                    range(
                        min(
                            max(configured_render_count, 1),
                            env_cfg.env.num_envs,
                        )
                    )
                )
            # Depth tasks keep a separate count for environments with cameras.
            # When only --num_envs is overridden, keep that count valid instead
            # of leaving its larger training default in place.
            if (
                hasattr(env_cfg.env, "num_camera_envs")
                and args.num_student is None
            ):
                env_cfg.env.num_camera_envs = min(
                    env_cfg.env.num_camera_envs,
                    env_cfg.env.num_envs,
                )
        if args.debug:
        ...
        if args.num_student is not None:
            if not 1 <= args.num_student <= env_cfg.env.num_envs:
                raise ValueError(
                    "--num_student must be between 1 and --num_envs "
                    f"({env_cfg.env.num_envs}), got {args.num_student}."
                )
            env_cfg.env.num_camera_envs = args.num_student

        if getattr(args, "distill_visual_smoke_test", False):
            if not hasattr(env_cfg, "distillation"):
                raise ValueError(
                    "--distill_visual_smoke_test requires a distillation task."
                )
            env_cfg.env.num_envs = 2
            if hasattr(env_cfg.env, "num_camera_envs"):
                env_cfg.env.num_camera_envs = 2
            env_cfg.viewer.rendered_envs_idx = [0, 1]
            env_cfg.terrain.num_cols = 2
            env_cfg.distillation.terrain_type_to_teacher = [1, 0]
            print(
                "Distillation visual smoke test enabled: "
                "2 environments, 2 cameras, terrain columns "
                "[stairs, gap]."
            )
...
def get_args():
    ...
    parser.add_argument(
        '--distill_visual_smoke_test',
        action='store_true',
        default=False,
        help=(
            "temporarily use two adjacent stairs/gap environments for "
            "visual inspection of a distillation task"
        ),
    )
