...
class K1DeepMimic(LeggedRobot):
    ...
    def _get_noise_scale_vec(self):
        ...
        noise_vec[3:7] = noise_scales.gravity * noise_level # projected gravity
        ...
        noise_vec[7 + 2*self.num_actions:43 + 2 * self.num_actions] = 0. # key body point position relative to base has no noise since it is important for the imitation task and the noise scale is hard to tune
        noise_vec[43 + 2*self.num_actions:43 + 3 * self.num_actions] = 0.  # previous actions
        noise_vec[43 + 3 * self.num_actions:46 + 3 * self.num_actions] = 0.  # ref_root_lin_vel
        noise_vec[46 + 3 * self.num_actions:49 + 3 * self.num_actions] = 0.  # ref_root_ang_vel
        noise_vec[49 + 3 * self.num_actions:53 + 3 * self.num_actions] = 0.  # ref_base_quat
        noise_vec[53 + 3 * self.num_actions:53 + 4 * self.num_actions] = 0.  # ref_dof_pos
        noise_vec[53 + 4 * self.num_actions:53 + 5 * self.num_actions] = 0.  # ref_dof_vel
        noise_vec[53 + 5 * self.num_actions:53 + 6 * self.num_actions] = 0.  # ref_key_body_pos_relative_to_base
        return noise_vec
