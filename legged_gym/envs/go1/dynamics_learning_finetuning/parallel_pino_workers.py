...
class SharedTensors:
    ...
    def __init__(self, num_envs, num_dof, foot_dim=(4,3)):
        ...
        self.q       = self.make("full_ft_q",       (num_envs, self.DOF+1))
        self.qd      = self.make("full_ft_qd",      (num_envs, self.DOF))
        self.qd_prev = self.make("full_ft_qd_prev", (num_envs, self.DOF))
        self.grf     = self.make("full_ft_grf",     (num_envs, *self.FOOT_DIM))
        self.dt      = self.make("full_ft_dt",      (1,))

        # Outputs
        self.wb_dynamics = self.make("full_ft_wb_dynamics", (num_envs, self.DOF))
        self.wb_contacts = self.make("full_ft_wb_contacts", (num_envs, self.DOF))
        self.mass_mat    = self.make("full_ft_mass_mat",    (num_envs, self.DOF, self.DOF))
        self.bias        = self.make("full_ft_bias",        (num_envs, self.DOF))
        self.acc6d       = self.make("full_ft_acc6d",       (num_envs, 6))
...
def compute_single_env(i, shared, pino_model, pino_data, 
    ...
    q       = shared.full_ft_q[i]
    qd      = shared.full_ft_qd[i]
    qdprev  = shared.full_ft_qd_prev[i]
    grf     = shared.full_ft_grf[i]      # shaped as (12)
    dt      = shared.full_ft_dt[0]

    # Acceleration (backward finite difference)
    acc = (qd - qdprev) / dt

    # 6-DoF torso accel
    shared.full_ft_acc6d[i] = acc[:6]
    ...
    wb_dyn = M @ acc + b
    shared.full_ft_wb_dynamics[i] = wb_dyn[correct_idxs]

    # Also save reduced mass matrix + bias
    M_ = M[np.ix_(correct_idxs, correct_idxs)]
    shared.full_ft_mass_mat[i] = M_
    shared.full_ft_bias[i] = b[correct_idxs]
    ...
    shared.full_ft_wb_contacts[i] = tau[correct_idxs]
