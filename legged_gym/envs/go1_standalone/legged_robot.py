...
class LeggedRobot(BaseTask):
    ...
    def _reset_root_states(self, envs_idx):
        ...
        if self.custom_origins:
            self.base_pos[envs_idx] = self.base_init_pos
            offset = self.env_origins[envs_idx]
            self.base_pos[envs_idx] += self.env_origins[envs_idx]
        ...
        self.robot.set_pos(
            self.base_pos[envs_idx], zero_velocity=False, envs_idx=envs_idx)
        new_particle_posistions = self.liquid_init_pos[envs_idx] + (self.base_pos[envs_idx] - self.base_init_pos)[:, None, :]
        active = torch.randint(0, len(self.liquids), (len(envs_idx),1))
        #print("JDHWHFIAWHFIFHIAWHFIHAWIFHIWFH")
        #print(envs_idx)
        for i, liquid in enumerate(self.liquids):    
            #print((active == i).squeeze())
            liquid.set_particles_active(active == i, envs_idx=envs_idx)
            liquid.set_particles_vel(0, envs_idx=envs_idx)
            liquid.set_particles_pos(new_particle_posistions, envs_idx=envs_idx)
        # base quat
    ...
    def _create_envs(self):
        ...
        self.bucket = self.scene.add_entity(
            ...
            morph=gs.morphs.Mesh(
                file="cube_2.stl",
                scale=(stl_scale + 0.01, stl_scale + 0.01, stl_scale + 0.01),    # adjust scale if needed
        ...
        box_init_pose = np.array(self.cfg.init_state.pos)
        box_init_pose[2] += bucket_offset + 0.01
        #box_init_pose[2] += bucket_offset + 0.1
        #self.scene.add_entity(
        #        material=gs.materials.SPH.Liquid(rho=liquid_mass),
        #        morph=gs.morphs.Box(pos=(0, 0, 0.577), 
        #                        size=(1,1,1)),
        ...
        #print(self.liquid_init_pos.shape)
        self.liquid_init_pos[:] = self.liquids[0].get_particles_pos()
        #print(self.bucket.get_pos())
